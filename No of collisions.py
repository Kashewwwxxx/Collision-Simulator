...
class MinHeap:
    def __init__(self):           # heap as a list
        self.heap = []
    # length of heap
    def __len__(self):
        return len(self.heap)

    def left(self, j):        # returns tge index of left of jth element
        return 2*j + 1

    def right(self, j):       # returns the index of right of jth element
        return 2*j + 2

    def parent(self,j):       # returns the index of root of jth  element
        return (j-1)//2

    def swap(self,i,j):       # this function simply swaps the the elements of heap
        self.heap[i], self.heap[j] = self.heap[j], self.heap[i]

    def DownHeap(self,j):     # if the max element is present in wrong place then it looks the situation and correspondingly make it to the bottom of the heap so as to make it a min heap.
    ...
    def heappush(self,key):            # pushing the element into the heap and sorting it simultaneously
        self.heap.append(key)
        self.UpHeap(len(self.heap)-1)

    def heappop(self):                 # this function will pop the element from the heap and returns the popped element.
        if (len(self)!= 0):
...
def listCollisions(M,x,v,m,t):            # this is our final function which return all the collisions
    ...
    global total_coll
    global collision
    update_lst = [0] * len(M)               # in this empty list all the collisions are appended
    exp_coll = Expected_coll(x, v)         # initial expected collisions
    time_of_collision = [0]*len(M)
    while total_coll <= m and total_time <= t:
        if exp_coll.heap != []:                     # only enter this condition when loop is not empty
            ...
            if update_lst[ball_i] == fst_coll[3]:        # this condition discards the previous expected collisions which will are not happening in the present.
                ...
                x[ball_i] = x[ball_i+1] = loc + vel*(tm-time_of_collision[ball_i])                # changin the positions x[i] and x[i+1]
                ...
                if ball_i-1 >= 0:                                                   # checking whether i-1th ball exists or not
                    ...
                    if v[ball_i-1] * v[ball_i] >= 0:                                 # checking the collision of i-1th and ith balls
                        ...
                        if v[ball_i-1] > 0:
                            ...
                            fst_coll[3] = update_lst[ball_i-1]                        # changing the 4th index of and making it equal to the i-1th index of update_lst
                            exp_coll.heappush([t1+tm, ball_i-1, Xi, fst_coll[3]])     # pushing the required collision tuple in expected collision list
                if ball_i+2 < len(v):                                                  # checking whether i+2th ball exists or not
                    loc_j = x[ball_i+2] + v[ball_i+2]*(tm-time_of_collision[ball_i+2])      # storing the changed value of i-1th ball which will be used in further calculations
                ...
                fst_coll[2] = round(fst_coll[2], 4)                               # rounding off the values of location of collision
                fst_coll[0] = round(fst_coll[0], 4)                               # rounding off the values of time of collision
                if total_coll <= m and total_time <= t:
                    collision.append((fst_coll[0],fst_coll[1],fst_coll[2]))        # appending the popped collision (from heap) in the collision list
            elif update_lst[ball_i] != fst_coll[3]:                                # discarding the unwanted previous collisions
                continue
        else:
            break                                                                 # finally breaking the loop when expected collision list is empty
    return collision                                                              # returning the collision list
# defining a heap class
class MinHeap:
    def __init__(self):           # heap as a list
        self.heap = []
